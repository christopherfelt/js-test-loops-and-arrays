...
describe("bonuses.js", () => {
  describe("1. letterChecker Function", () => {
    ...
    it("returns the missing letter", () => {
      chai.assert.strictEqual(
        letterChecker("qrsuv"),
        "t",
        "You must return the missing letter."
      );
    });
    it('returns "no missing letters" if all letters are accounted for', () => {
      chai.assert.strictEqual(
        letterChecker("cdefghi"),
        "no missing letters",
        "If all letters are account for, return 'no missing letters'."
      );
    });
  });
  describe("2. sumEqualsTarget Function", () => {
    ...
    it("returns a bool", () => {
      chai.assert.isBoolean(
        sumEqualsTarget([1, 1], 2),
        "The return type must be a bool."
      );
    });
    it("returns true if a pair of numbers' sum equals the target number", () => {
      chai.assert.strictEqual(
        sumEqualsTarget([2, 6, 4, 3], 7),
        true,
        "If any pair's sum equals the target return true."
      );
    });
    it("processes large data inputs quickly", function () {
      this.timeout(50);
      chai.assert.isBoolean(
        sumEqualsTarget(bigArr, 42),
        "You have 50ms to process an array of 1 million numbers. Good luck!"
      );
    });
  });
  describe("3. oddAscender Function", () => {
    ...
    it("don't change the position of even numbers", () => {
      chai.assert.isTrue(
        arraysMatch(oddAscender([2, 8, 4]), [2, 8, 4]),
        "Only change the position of odd numbers!"
      );
    });
    it("sorts odd numbers ascending", () => {
      chai.assert.isTrue(
        arraysMatch(oddAscender([2, 9, 4, 3, 6, 6, 1, 5, 8]), [
          2,
          1,
          4,
          3,
        ...
        "Return the array with evens in same positions and odds ascending."
      );
    });
  });
});
